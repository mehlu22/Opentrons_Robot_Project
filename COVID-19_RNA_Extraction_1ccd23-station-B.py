def get_values(*names):
    import json
    _all_values = json.loads("""{"num_samples":96,"mag_height":6.8,"magplate_offset":1,"radial_offset":0.8,"starting_vol":15,"binding_buffer_vol":15,"wash1_vol":15,"wash2_vol":15,"wash3_vol":15,"elution_vol":15,"mix_reps":15,"settling_time":1,"park_tips":true,"tip_track":false,"flash":false}""")
...
def run(ctx):
    ...
    magdeck.disengage()
    magplate = magdeck.load_labware('corning_24_wellplate_3.4ml_flat')
    ...
    res1 = ctx.load_labware(
        'nest_12_reservoir_15ml', '6', 'reagent reservoir 1')
    num_cols = math.ceil(num_samples/8)
    tips1000 = [ctx.load_labware('opentrons_96_tiprack_1000ul', slot,
                                '1000µl filtertiprack')
               for slot in ['4', '11']]
    if park_tips:
        parkingrack = ctx.load_labware(
            'opentrons_96_tiprack_1000ul', '8', '200µl filtertiprack (parking)')
        tips1000.insert(0, parkingrack)
        parking_spots = parkingrack.rows()[0][:num_cols]
    else:
        tips1000.insert(0, ctx.load_labware('opentrons_96_tiprack_1000ul', '7',
                                           '200µl filtertiprack'))
    ...
    #radius = mag_samples_m[0].geometry.width/2
    ...
    tip_log['tips'] = {
        m300: [tip for rack in tips1000 for tip in rack.rows()[0]]}
...
before resuming.')
        ...
        for i, (m, spot) in enumerate(zip(mag_samples_m, parking_spots)):
            ...
            loc = m.bottom().move(Point(
                x=side*radial_offset, z=magplate_offset))
            for _ in range(num_trans):
                ...
                m300.transfer(vol_per_trans, loc, loc, new_tip='never',
                              air_gap=20)
                m300.blow_out(loc)
                m300.air_gap(20)
            _drop(m300)
