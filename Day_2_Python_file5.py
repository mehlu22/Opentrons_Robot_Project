...
#metadata
metadata = {
    'protocolName': "3' mRNA Next Gen Sequencing Protocol File 5",
    'author': 'Mehlam M Saifudeen <mehlu22@gmail.com>',
    'description': 'Purification 2',
    'apiLevel': '2.12'
}
...
#Purification using magnetic beads
    ...
    while(copy_2!=0):
        if(j>12):
            ...
            j+=1
        p1000.pick_up_tip()
        p1000.aspirate(dataset.loc[15, 'amount-uL'], temp_plate['{0}'.format(dataset.loc[15, 'slot'])], rate = 0.4)
        p1000.dispense(dataset.loc[15, 'amount-uL'], magnetic_plate['{0}{1}'.format(rows[i], columns[j])], rate = 0.4)
        p1000.mix(8, 25, magnetic_plate['{0}{1}'.format(rows[i], columns[j])], rate = 0.4)
    ...
    while(copy_3!=0):
        if(j>12):
            ...
            j+=1
        p1000.pick_up_tip()
        p1000.aspirate(47, magnetic_plate['{0}{1}'.format(rows[i], columns[j])], rate = 0.4)  #TODO Modu
        p1000.drop_tip()
        copy_3 = copy_3 - 1
    
    
    #Step 3 - Add 30ul EB, mix well, and incubate for 2 mins
    magnetic_module.disengage()
    i=0
    j=-1
    while(copy_4!=0):
        if(j>12):
            ...
            j+=1
        p1000.pick_up_tip()
        p1000.aspirate(dataset.loc[16, 'amount-uL'], tuberack_1['{0}'.format(dataset.loc[16, 'slot'])], rate = 0.4)
        p1000.dispense(dataset.loc[16, 'amount-uL'], magnetic_plate['{0}{1}'.format(rows[i], columns[j])], rate = 0.4)
    ...
    #Step 4 - Add 30ul of PS, mix well and incubate for 5 mins
    i=0
    j=-1
    while(copy_5!=0):
        if(j>12):
            ...
            j+=1
        p1000.pick_up_tip()
        p1000.aspirate(dataset.loc[17, 'amount-uL'], tuberack_1['{0}'.format(dataset.loc[17, 'slot'])], rate = 0.4)
        p1000.dispense(dataset.loc[17, 'amount-uL'], magnetic_plate['{0}{1}'.format(rows[i], columns[j])], rate = 0.4)
    ...
    while(copy_6!=0):
        if(j>12):
            ...
            j+=1
        p1000.pick_up_tip()
        p1000.aspirate(60, magnetic_plate['{0}{1}'.format(rows[i], columns[j])], rate = 0.4)
        p1000.drop_tip()
        copy_6 = copy_6 - 1
    ...
    while(copy_9!=0):
        if(j>12):
            ...
            j+=1
        p1000.pick_up_tip()
        p1000.aspirate(dataset.loc[18, 'amount-uL'], tuberack_1['{0}'.format(dataset.loc[18, 'slot'])], rate = 0.4)
        p1000.dispense(dataset.loc[18, 'amount-uL'], magnetic_plate['{0}{1}'.format(rows[i], columns[j])], rate = 0.4)
        p1000.blow_out(magnetic_plate['{0}{1}'.format(rows[i], columns[j])])
        protocol.delay(seconds = 5) #Keep at 30 secs
        p1000.aspirate(dataset.loc[18, 'amount-uL'], magnetic_plate['{0}{1}'.format(rows[i], columns[j])], rate = 0.4)
        p1000.drop_tip()
        p1000.pick_up_tip()
        p1000.aspirate(dataset.loc[18, 'amount-uL'], tuberack_1['{0}'.format(dataset.loc[18, 'slot'])], rate = 0.4)
        p1000.dispense(dataset.loc[18, 'amount-uL'], magnetic_plate['{0}{1}'.format(rows[i], columns[j])], rate = 0.4)
    ...
    while(copy_7!=0):
        if(j>12):
            ...
            j+=1
        p20.pick_up_tip()
        p20.aspirate(dataset.loc[19, 'amount-uL'], tuberack_1['{0}'.format(dataset.loc[19, 'slot'])], rate = 0.4)
        p20.dispense(dataset.loc[19, 'amount-uL'], magnetic_plate['{0}{1}'.format(rows[i], columns[j])], rate = 0.4)
        p20.mix(8, 18, magnetic_plate['{0}{1}'.format(rows[i], columns[j])], rate = 0.4)
    ...
    while(copy_8!=0):
        ...
        protocol.home()
        protocol.pause('This is the end of the final part of the protocol')
