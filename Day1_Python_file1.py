...
#metadata
metadata = {
    'protocolName': "3' mRNA Next Gen Sequencing Protocol File 1",
    'author': 'Mehlam M Saifudeen <mehlu22@gmail.com>',
    'description': 'To generate the first strand cDNA synthesis using reverse transcription',
    'apiLevel': '2.12'
}
...
#     p20.flow_rate.dispense = 100
    ...
    #Implementing the protocol part 1
    rows = "ABCDEFGH"
    ...
    while(copy != 0):
        if(j>12):
            ...
            j+=1
        p20.pick_up_tip()
        p20.aspirate(dataset.loc[0, 'amount-uL'], temp_plate['{0}'.format(dataset.loc[0, 'slot'])], rate = 0.4)
        p20.dispense(dataset.loc[0, 'amount-uL'], temp_plate_2['{0}{1}'.format(rows[i], columns[j])], rate = 0.4)
        p20.blow_out()
        p20.drop_tip()
        copy = copy - 1
    
    #Increasing the temperature of the module to 85C and incubating for 3 minutes
    temp_module_2.set_temperature(35.0) #Keep at 85
    protocol.delay(minutes = 0, seconds = 10) #Keep at 3 mins 30 secs
    temp_module_2.set_temperature(42.0)
    #temp_module_2.await_temperature(42.0)
    
    #Making the mastermix 1 containing FS2 and E1
    p1000.pick_up_tip()
    p1000.aspirate(dataset.loc[1, 'amount-uL']*(num_of_experiments+1), temp_plate['{0}'.format(dataset.loc[1, 'slot'])], rate = 0.4)
    p1000.dispense(dataset.loc[1, 'amount-uL']*(num_of_experiments+1), temp_plate['A6'], rate = 0.4)
    p1000.blow_out()
    p1000.drop_tip()
    p20.pick_up_tip()
    p20.aspirate(dataset.loc[2, 'amount-uL']*(num_of_experiments+1), temp_plate['{0}'.format(dataset.loc[2, 'slot'])], rate = 0.4)
    p20.dispense(dataset.loc[2, 'amount-uL']*(num_of_experiments+1), temp_plate['A6'], rate = 0.4)
    p20.blow_out()
    p20.drop_tip()
    temp_module_2.set_temperature(42.0)
    protocol.delay(minutes = 1, seconds = 30) #Keep at 2 mins 30 secs
    p1000.pick_up_tip()
    p1000.mix(8, 10*num_of_experiments, temp_plate['A6'], rate = 0.4)
    p1000.blow_out()
    p1000.drop_tip()
    
    #Adding the mastermix to the pcr plate
    i=0
    j=-1
    while(copy_2!=0):
        if(j>12):
            ...
            j+=1
        p20.pick_up_tip()
        p20.aspirate(10, temp_plate['A6'], rate = 0.4)
        p20.dispense(10, temp_plate_2['{0}{1}'.format(rows[i], columns[j])], rate = 0.4)
        p20.mix(8, 9, temp_plate_2['{0}{1}'.format(rows[i], columns[j])], rate = 0.4)
        p20.blow_out()
        p20.drop_tip()
        copy_2 = copy_2 - 1
    
    temp_module_2.set_temperature(42.0)
    protocol.delay(minutes = 1, seconds = 0) #Keep at 15 mins
    ...
    while(copy_3!=0):
        if(j>12):
            ...
            j+=1
        p20.pick_up_tip()
        p20.aspirate(dataset.loc[3, 'amount-uL'], temp_plate['{0}'.format(dataset.loc[3, 'slot'])], rate = 0.4)
        p20.dispense(dataset.loc[3, 'amount-uL'], temp_plate_2['{0}{1}'.format(rows[i], columns[j])], rate = 0.4)
        p20.mix(8, 18, temp_plate_2['{0}{1}'.format(rows[i], columns[j])], rate = 0.4)
        p20.blow_out()
        p20.drop_tip()
        copy_3 = copy_3 - 1
    
    temp_module_2.set_temperature(45.0) #Keep at 95
    protocol.delay(minutes = 0, seconds = 10) #Keep at 10 mins
    temp_module_2.set_temperature(25.0)
    protocol.delay(minutes = 0, seconds = 10) #Keep at 5 mins
    
    #Adding the SS1 followed by pausing the protocol until part 2 is implemented
    i=0
    j=-1
    while(copy_4!=0):
        if(j>12):
            ...
            j+=1
        p20.pick_up_tip()
        p20.aspirate(dataset.loc[4, 'amount-uL'], temp_plate['{0}'.format(dataset.loc[4, 'slot'])], rate = 0.4)
        p20.dispense(dataset.loc[4, 'amount-uL'], temp_plate_2['{0}{1}'.format(rows[i], columns[j])], rate = 0.4)
        p20.mix(8, 18, temp_plate_2['{0}{1}'.format(rows[i], columns[j])], rate = 0.4)
        p20.blow_out()
        p20.drop_tip()
        copy_4 = copy_4 - 1
        ...
        protocol.home()
        protocol.pause('This is the end of the first part of the protocol')
